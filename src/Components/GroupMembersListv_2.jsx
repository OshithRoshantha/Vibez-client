...
export default function GroupMemberList({ darkMode }) {
  ...
  const toggleDeleteGroupPopup = () => {
    setDeleteGroupPopup((prevState) => !prevState);
  };

  const toggleRemoveMemberPopup = () => {
    setRemoveMemberPopup((prevState) => !prevState);
  };

  const groupName = "friends";
  const memberName = "user01";
  ...
  return (
    <div className="mt-0">
      {deleteGroupPopup && (
        <GlobalAlert
          darkMode={darkMode}
          text={`Delete "${groupName}" group?`}
          textOP="Deleting this group will remove it permanently for all members."
          button1="Cancel"
          button2="Delete group"
          btn1Function={toggleDeleteGroupPopup}
          btn2Function={toggleDeleteGroupPopup}
        />
      )}
      {removeMemberPopup && (
        <GlobalAlert
          darkMode={darkMode}
          text={`Remove ${memberName} from "${groupName}" group?`}
          textOP="This action cannot be undone."
          button1="Cancel"
          button2="Remove"
          btn1Function={toggleRemoveMemberPopup}
          btn2Function={toggleRemoveMemberPopup}
        />
      )}
      <div className="flex items-center mb-2">
        ...
        <button
          onClick={toggleDeleteGroupPopup}
      ...
      <div className="flex items-center mb-2">
        ...
        <button
          onClick={toggleRemoveMemberPopup}
          className={`${
            darkMode
